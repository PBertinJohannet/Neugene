...
impl LearnParams {
    pub fn new(nb_problems : usize,test_per_prob : usize,
               max_gen : usize, starting_coef : f64, coef_mod : f64, percent_elite : f64) -> Self {
        LearnParams{
...
impl<P : ManyStepProblem> ReiLearn<P>{
    pub fn new(net : Network, prob_conf : P::ProblemConfig, learn_param : LearnParams)-> Self {
        ...
        ReiLearn {
            ...
            test_problems: (0..TEST_DATA_SIZE).map(|_| P::random(&mut my_rand, &prob_conf)).collect(),
            net: (net),
            random: (my_rand),
            params : learn_param,
    ...
    pub fn demonstrate(&self){
        for p in self.test_problems.iter(){
            ...
            for _ in 0..p.max_step().unwrap_or(30){
                prob.print_state();
    ...
    pub fn gen_tests_from_choices(&mut self, mut games : Vec<(f64, Vec<Choice>)>) -> Vec<Test>{
        ...
        games.into_iter().enumerate().map(|(index, (_, choices))| {
            if index < lower {
                choices.into_iter().map(|c| c.into_bad_test()).collect::<Vec<Test>>()
            } else if index > upper{
                choices.into_iter().map(|c| c.into_good_test()).collect::<Vec<Test>>()
            } else {
                vec![]
            }
        }).flatten().collect()
